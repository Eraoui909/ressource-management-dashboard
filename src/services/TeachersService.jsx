...
export const addNewTeacher = async (teacher) =>{
  ...
  await axios.post(TEACHER_API_URL+"/teachers/add",teacher).then(
    (response) =>{
      return response;
    }
  ).catch(
...
export const getDemand = async () =>{
  let {data:demand} = await axios.get(OLD_TEACHER_API_URL+"/demands/" + JSON.parse(localStorage.getItem("user")).email)
